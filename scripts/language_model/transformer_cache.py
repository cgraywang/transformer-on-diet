...
class CacheCell(Block):
    ...
    """
    def __init__(self, lm_model, vocab_size, window, theta, lambdas, cache_output=False, **kwargs):
    ...
    def __call__(self, inputs, char_inputs, target,
                next_word_history, cache_history, valid_length=None):  #pylint: disable=arguments-differ
        ...
        """
        return super(CacheCell, self).__call__(inputs, char_inputs, target,
                next_word_history, cache_history, valid_length)

    def forward(self, inputs, char_inputs, target,
                next_word_history, cache_history, valid_length=None): # pylint: disable=arguments-differ
        ...
        cache_history: NDArray
            ...
            (size is equal to the window size)
        """
        #TODO: add encoder_hs from attention lm, check encoder_hs.shape
        output, _, encoder_hs, _ = self.lm_model(inputs, char_inputs, valid_length=valid_length)
        if self._cache_output:
            raw_output = output.softmax(axis=-1).argmax(axis=-1)
        ...
        if not self._cache_output:
            ...
            next_word_history = nd.concat(
                *[nd.one_hot(t[0], self._vocab_size, on_value=1, off_value=0)
                  for t in raw_output], dim=0) if next_word_history is None \
                else nd.concat(next_word_history,
                               nd.concat(
                                   *[nd.one_hot(t[0], self._vocab_size, on_value=1, off_value=0)
                                     for t in raw_output], dim=0), dim=0)
        ...
        for idx, vocab_L in enumerate(softmax_output):
            ...
            if not self._cache_output:
                ...
                out = joint_p[raw_output[idx]] if out is None \
                    else nd.concat(out, joint_p[raw_output[idx]], dim=0)
